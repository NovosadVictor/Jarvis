...
def home_detail(request, pk, home_id):
    args = {}
    try:
...
def room_detail(request, home_id, room_id):
    ...
    try:
        ...
        if home.house_keeper == request.user:
...
def insert_room(request, home_id):
    ...
    form = InsertRoomForm()
    try:
        home = Home.objects.select_related().get(id=home_id)
        args['home'] = home
        ...
        if home.house_keeper == request.user:
            if request.POST:
                ...
                if room_form.is_valid():
                    ...
                    new_room.save()
                    return redirect('/homes/%d/' % home.house_keeper.id + 'homes_page/%d' % home.id)
                else:
...
def insert_device(request, room_id):
    ...
    try:
        room = Room.objects.select_related('Home').get(id=room_id)
        args['room'] = room
        args['home'] = room.home
        if room.home.house_keeper == request.user:
            if request.POST:
...
def update_values(request, device_id):
    ...
    try:
        device = Device.objects.select_related().get(id=device_id)
        args['device'] = device
        args['room'] = device.room
        args['home'] = device.room.home
        if device.room.home.house_keeper == request.user:
            if request.POST:
                ...
                if update_form.is_valid():
                    ...
                    device.mode = mode
                    device.save()
                    return redirect('/homes/%d/' % device.room.home.id + 'room_page/%d' % device.room.id)
                else:
                    args['logic_error'] = 'There are something wrong'
            else:
                args['logic_error'] = 'You cant be here'
